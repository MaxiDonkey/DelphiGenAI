...
- [Gpt-image-2 model](#gpt-image-2-model)
- [Create image edit with gpt-image-2](#create-image-edit-with-gpt-image-2)
...
Since April 21, 2026, OpenAI has offered the `gpt-image-2` model for image creation and editing. This state-of-the-art model delivers higher quality and stronger instruction following than `gpt-image-1` and `dall-e-3`, and natively supports flexible image sizes and high-fidelity image inputs.

In the configuration, you now have four additional parameters for image generation:
- **background:** Allows you to set the transparency of the generated image’s background. Must be one of `transparent`, `opaque`, or `auto` (default). This parameter has been **removed** from `gpt-image-2`, but it is still available with the `gpt-image-1.5` model.

> [!NOTE]
> It is a shame that this option was dropped from `gpt-image-2`, as it was very useful.

- **moderation:** Controls the content-moderation level for images generated by `gpt-image-2`. Must be either `low` (less restrictive filtering) or `auto` (default).

- **output_compression:** Specifies the compression level (0–100%) for the generated images. Only supported by `gpt-image-2` when using the `webp` or `jpeg` output formats; defaults to 100.

- **output_format:** Determines the format in which generated images are returned. Only supported by `gpt-image-2`. Must be one of `png`, `jpeg`, or `webp`.

<br>

Additionally, several existing parameters have been extended with new values for `gpt-image-2`:

quality: Supports `high`, `medium`, `low`, and `auto`.

size: Supports named sizes such as `1024x1024`, `1536x1024` (landscape), `1024x1536` (portrait), `2048x2048`, `2048x1152`, `3840x2160`, `2160x3840`, or `auto` (default). `gpt-image-2` also accepts arbitrary `WIDTHxHEIGHT` resolutions, provided both dimensions are multiples of 16, the aspect ratio stays between 1:3 and 3:1, and the longest edge does not exceed 3840px.

prompt: Allows up to 32,000 characters for `gpt-image-2` (versus 1,000 for `dall-e-2` and 4,000 for `dall-e-3`).

<br>

An example of image creation with gpt-image-2 (Asynchronous because response times are much longer):

```pascal
//uses GenAI, GenAI.Types, GenAI.Tutorial.VCL;

  TutorialHub.FileName := 'GptImage2.png';
...
### Create image edit with gpt-image-2
...
However, with `gpt-image-2`, the output quality is significantly higher.
...
3. Compose your extended prompt
     - In your request, describe exactly what the model should insert into the masked area. You now have up to 32,000 characters for a fully detailed description.
...
Below, you’ll find an example of the code to send to gpt-image-2 to initiate the edit.
